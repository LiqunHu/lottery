...
class SpiderSports500WBatch(BatchBase):
    def run(self):
        ...
        maxDate = self.session.query(func.max(MatchInfo500.date).label('max_date')).first()
    ...
    def getWDLRate(self, tdInfo):
        ...
        if len(spans) == 3:
            rate = (float(spans[0]['data-sp']), float(spans[1]
                                                      ['data-sp']), float(spans[2]['data-sp']))

        spansF = divS[1].findAll('span')
        if len(spansF) == 3:
            rateF = (float(spansF[0]['data-sp']), float(spansF[1]
                                                        ['data-sp']), float(spansF[2]['data-sp']))
        return [rate, rateF]
    ...
    def getMDate(self, date, TimeStr):
        ...
        if not timeT:
            patternTime = re.compile(u'\d{2}:\d{2}');
            timeT = patternTime.findall(TimeStr)
        if len(timeT) > 0:
            d = date
            if timeT[0] > '23:40:00':
