...
class getAccountLogResource(ServiceBase):
    def on_get(self, req, resp):
        ...
        accountMsg = {}
        try:
            result = self.session.execute(text('select basemoney from tbl_userdata where userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['baseMoney'] = result[0]

            result = self.session.execute(text('select expdate from tbl_user where userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['expdate'] = result[0].isoformat()

            result = self.session.execute(text('select min(date) from tbl_account_running where status = "1" and userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['fromDate'] = result[0].isoformat()

            result = self.session.execute(text('select max(date) from tbl_account_running where status = "1" and userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['toDate'] = result[0].isoformat()

            result = self.session.execute(text('select max(usemoney) from tbl_account_running where status = "1" and  userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['maxUse'] = result[0]

            result = self.session.execute(text('select sum(usemoney) from tbl_account_running where status = "1" and userid = :uid'), {
                                          'uid': req_para['userid']}).first()
            accountMsg['sumUse'] = result[0]

            result = self.session.execute(text(
                'select truncate(sum(usemoney + dResult),2) from tbl_account_running where status = "1" and userid = :uid'), {'uid': req_para['userid']}).first()
            accountMsg['sumWin'] = result[0]

            result = self.session.execute(text(
                'select max(usemoney)*0.08 from tbl_account_running where status = "1" and userid = :uid'), {'uid': req_para['userid']}).first()
