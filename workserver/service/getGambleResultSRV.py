        ...
        if u.accounttype == GLBConfig.ATYPE_GROUP:
            ...
            if udata.mode == GLBConfig.MODE_A:
                ...
                if 'matchA' in req_para.keys():
                    self.getMatchModeASelfChoice(
                        u, udata, req_para['matchA'], req_para['AResult'], req_para['matchB'], req_para['BResult'])
...
#                       and_(MatchInfoD.date==SysUtil.getToday(),MatchInfoD.matchTime >= datetime.time(22,00,00)) )).\
    ...
    def getMatchModeA(self, u, udata, dealerid):
        ...
        rate = listA[SysUtil.getResultIndex(
            matches.matchAResult)] * listB[SysUtil.getResultIndex(matches.matchBResult)]
    ...
    def getMatchModeASelfChoice(self, u, udata, matchA, AResult, matchB, BResult):
        ...
        rate = listA[SysUtil.getResultIndex(
            AResult)] * listB[SysUtil.getResultIndex(BResult)]
        mMoney = (udata.basemoney - money) / (rate - 1.00)
...
#                       and_(MatchInfoD.date==SysUtil.getToday(),MatchInfoD.matchTime >= datetime.time(22,00,00)) )).\
    ...
    def getMatchModeC(self, u, ud, matchid):
        ...
        count = 1
        md1 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_HAFU,
                        matchAID=m.matchid, matchAResult='dw', rate=m.dw)
        self.session.add(md1)
        md2 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_HAFU,
                        matchAID=m.matchid, matchAResult='dd', rate=m.dd)
        self.session.add(md2)
        md3 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_HAFU,
                        matchAID=m.matchid, matchAResult='dl', rate=m.dl)
    ...
    def getMatchModeD(self, u, ud, matchid):
        ...
        count = 1
        md1 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_SCORE,
                        matchAID=m.matchid, matchAResult='2', rate=m.s2)
        self.session.add(md1)
        md2 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_SCORE,
                        matchAID=m.matchid, matchAResult='3', rate=m.s3)
        self.session.add(md2)
        md3 = MatchData(userid=u.userid, date=tomorrow, singleFlag=GLBConfig.M_SCORE,
                        matchAID=m.matchid, matchAResult='4', rate=m.s4)
        self.session.add(md3)
        self.session.flush()
        if count > 0:
            self.calMoney(u, ud, count)
