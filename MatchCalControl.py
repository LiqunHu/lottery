...
def getMatchDraw(date):
    ...
    if matches:
        for m in matches:
            md1 = MatchDataBatch(date=date, singleFlag='3',
                                 matchAID=m.matchid, matchAResult='dw', rate=m.dw)
            session.add(md1)
            md2 = MatchDataBatch(date=date, singleFlag='3',
                                 matchAID=m.matchid, matchAResult='dd', rate=m.dd)
            session.add(md2)
            md3 = MatchDataBatch(date=date, singleFlag='3',
...
if __name__ == '__main__':
    ...
    for date in dateRange('2016-10-24', '2017-02-01'):
        ...
        account = session.query(AccountRunningBatch).order_by(
            AccountRunningBatch.date.desc()).first()
        if account:
            if account.riskMoney < 0:
                money = ((-1) * account.riskMoney / (basemoney) + 1) * \
                    basemoney * matchCount * 1.5
